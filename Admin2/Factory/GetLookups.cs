    ...
    public class GetLookups
    {
        ...
        public DropdownModel GeLookupJakataMasterFilter()
        {
            using (var client = new System.Net.Http.HttpClient())
            {
                ...
                var env = ConfigurationManager.AppSettings["Enviroment"];
                var settings = "";
                var uri = new Uri("http://api.storyteller.today/api/JakataMasterFilter/");


                if (env == "Dev")
                {
                    settings = ConfigurationManager.AppSettings["LocalWebApi"];
                    uri = new Uri("http://localhost:5199/api/JakataMasterype/JakataMasterFilter/");
                ...
                {
                    settings = ConfigurationManager.AppSettings["ProductionWebApi"];
                    uri = new Uri("http://api.storyteller.today/api/JakataMasterype/JakataMasterFilter/");
